...
while not exit_:
	...
	for id_parent in tqdm(ids_):					   								   #Here we iterate through all the leaves of our tree, creating an ever
		...
		i=0
		for row in range(4):                                      					   #Iterating across every matrix cell
			for col in range(3):
				if parent_matrix[row][col] != 0:                                       #Checking if peice
					...
					for move in moves:
						...
						if child_matrix[new_row][new_col] == 0:
							...
							try:
								if not areSame(child_matrix, tree[id_parent[:-1]].data):
							...
							i+=1
						if areSame(child_matrix, final_matrix):
							print(final_matrix)
							exit_ = True
